...
- 工作日 16:30（Asia/Shanghai）运行；
- 支持手动 `workflow_dispatch`；
- 行情使用新浪/腾讯双源；
- 财务/估值来自东方财富公开接口；
- 只有完整收盘数据通过质量 Gate 才写入仓库；
- K 线采用滚动式更新：每个交易日追加当天收盘 OHLCV，按交易日去重，并且每只股票只保留最近 180 个交易日；
- 5/20/60/120/180 日结构都从同一份 180 日滚动历史中派生，不维护多套历史库；
- 当写入第 181 个交易日时自动淘汰最旧的一天，仓库不会因为每日更新而无限增长；
