...
- 同行业三维非补偿比较；
- 识别业务异质性与不可比性；
- 判断 PE / PB / ROE / 增长之间是否匹配；
- 判断经营改善质量与可持续性；
- 公开资料验证真实主营、核心盈利驱动和利润来源；
- 周期位置、一次性收益与正常化盈利判断；
- 最终估值、安全区、上下行空间和参与时机。
...
候选表已经只包含通过程序结构硬筛的公司，并已合并申万三级行业状态、估值、经营质量和价格结构字段。
...
公开资料只用于候选公司的 deep research，不得据此扩展程序生成的候选全集。
...
候选表已经通过结构硬筛，因此模型不得再次用结构硬规则缩小候选范围，也不得因为市场风险高而跳过候选。
...
- `position_pct`
- 支撑区 low / high / touches
- 成交密集区 low / high / volume share
- 阻力区
- `break_state` / invalidation

回答：**谁的当前价格结构更接近潜在承接区域，且结构风险更可控。**
...
PE / PB 不得孤立比较。低 PE 可能来自周期盈利高点；较高 PB 如果对应更高且可持续 ROE，也不能机械认定更贵。

回答：**当前估值是否得到盈利能力、增长和资产回报支撑。**
...
- 因为市场 `high risk` 而缩小已经生成的 model-ready 候选研究范围。

对任何因同行明确支配而排除的候选，必须能说明 `dominated_by` 以及三个维度的比较依据。
...
市场 `high risk` 可以让最终推荐更保守，例如要求更可信的安全边际、更严格的正常化估值、更倾向等待而非追价；但不得改变程序候选全集，也不得缩小三维比较和 Deep Research 应覆盖的范围。
...
- 公司逻辑成立，但当前价格离最终安全区仍偏远；
- 价格结构较低，但基本面 / 行业逻辑仍需确认；
- 估值暂时无法可靠确定但仍值得观察。

`research_uncertain` 必须说明最关键未解决问题；`excluded` 必须有明确理由。
...
其中程序直接给出前两个候选数量；模型只负责从 model-ready candidates 开始后的研究漏斗。

审计指标用于解释研究过程，不作为额外 Gate。
